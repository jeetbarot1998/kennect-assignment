function AddClassDynamically() {
    let johnElem = document.getElementById('john')
    let classNameToAppendFor = 'important'
    let classToAdd = 'shape'
    for (val of johnElem.children) {
        if (val.className.toUpperCase().includes(classNameToAppendFor.toUpperCase())) {
            val.classList.add(classToAdd)
            console.log('Class"', classToAdd, '"Added')
        }
    }
}
